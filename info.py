...
BOT_TOKEN = environ.get('BOT_TOKEN', '6217612807:AAFCoBBGrt-1ffAL1JqXso6P_fTnF8ciK70')
...
# Bot Admins
ADMINS = [int(admins) if id_pattern.search(admins) else admins for admins in environ.get('ADMINS', '5007198306').split()]
auth_users = [int(auth_users) if id_pattern.search(auth_users) else auth_users for auth_users in environ.get('AUTH_USERS', '5007198306').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []

# Channels
INDEX_CHANNELS = [int(index_channels) if id_pattern.search(index_channels) else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1001782414786').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1001720584562')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001929713367'))

# MongoDB information
DATABASE_URL = environ.get('DATABASE_URL', "mongodb+srv://dare12426:password1234@cluster0.xqxqbm7.mongodb.net/?retryWrites=true&w=majority")
...
WELCOME_MSG = is_enabled((environ.get('WELCOME_MSG', "False")), False)
